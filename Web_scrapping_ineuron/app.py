...
lg.basicConfig(filename='info.log', level=lg.INFO, format="%(asctime)s  %(levelname)s  %(message)s")
...
@app.route('/displaycourses',methods=['GET','POST']) #Display result page
@cross_origin()
def displaycourses():
    try:
        result={}
...
@app.route('/dboperation',methods=['GET','POST'])
@cross_origin()
def dbpage():
    return render_template('dboperation.html')


@app.route('/dbinsert',methods=['GET','POST']) # DB Operation
@cross_origin()
def mongoinsert():
    if request.method=='POST':
        try:
            ...
            print("Error while creating pymongo client, Check the logs")
        ...
        try:
            ...
            col = db[col_name]
            col.insert_one(data)
            lg.info('Data added Successfully : ')
